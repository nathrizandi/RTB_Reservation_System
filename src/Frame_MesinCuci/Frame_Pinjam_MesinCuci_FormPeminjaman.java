...
public class Frame_Pinjam_MesinCuci_FormPeminjaman extends JFrame implements ActionListener,WindowListener{
	...
	private JLabel lbl_header = new JLabel("Peminjaman Mesin Cuci");
	...
	private JRadioButton mesin1 = new JRadioButton("1 Mesin");
	private JRadioButton mesin2 = new JRadioButton("2 Mesin");
	private JRadioButton mesin3 = new JRadioButton("3 Mesin");
	private JRadioButton mesin4 = new JRadioButton("4 Mesin");
	private JRadioButton mesin5 = new JRadioButton("5 Mesin");
	
	private Frame_Pinjam_MesinCuci_ListPeminjaman list_peminjaman;
	private Main main;
	...
	public void init() {
		...
		radio_group.add(mesin1);
		radio_group.add(mesin2);
		radio_group.add(mesin3);
		radio_group.add(mesin4);
		radio_group.add(mesin5);
		pnl_center.add(mesin1);
		pnl_center.add(mesin2);
		pnl_center.add(mesin3);
		
		pnl_center.add(new JLabel(" "));
		pnl_center.add(mesin4);
		pnl_center.add(mesin5);
	...
	public Frame_Pinjam_MesinCuci_FormPeminjaman(Main main) {
		this.main =  main;
		init();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj.equals(btn_back)) {
			this.dispose();
			list_peminjaman = new Frame_Pinjam_MesinCuci_ListPeminjaman(main);
			...
			if(
				...
				|| !mesin1.isSelected() && !mesin2.isSelected() && !mesin3.isSelected() && !mesin4.isSelected() && !mesin5.isSelected()
					) {
			...
			if(mesin1.isSelected()) {
				jumlah_tungku = 1;
			}else if(mesin2.isSelected()) {
				jumlah_tungku = 2;
			}else if(mesin3.isSelected()) {
				jumlah_tungku = 3;
			}else if(mesin4.isSelected()) {
				jumlah_tungku = 4;
			}else if(mesin5.isSelected()) {
				jumlah_tungku = 5;
			}
			...
			Mesincuci temp = new Mesincuci(jumlah_tungku, hari, bulan, tahun, jam_mulai, menit_mulai, jam_selesai, menit_selesai,main.getCurr_peminjam().getNama() , main.getCurr_peminjam().getKelas(), main.getCurr_peminjam().getGender());
			...
			if(main.isDate(localDate) && main.isDuration(jam_mulai, menit_mulai, jam_selesai, menit_selesai) && main.valid_tanggal_mesincuci(temp)) {
					success_pop.showMessageDialog(null, "Jadwal berhasil ditambahkan", "Informasi", JOptionPane.INFORMATION_MESSAGE);
					Frame_Pinjam_MesinCuci_ListPeminjaman n = new Frame_Pinjam_MesinCuci_ListPeminjaman(main);
					this.dispose();
				...
				else if(!main.valid_tanggal_mesincuci(temp)) {
